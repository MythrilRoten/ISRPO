...
class MainWindow(QMainWindow):
    ...
    DATA_DIR = os.path.join(BASE_DIR, 'main')
    MEDIA_DIR = os.path.join(DATA_DIR, 'photos')
    DOCX_DIR = os.path.join(DATA_DIR, 'maket.docx')
    LIB_DIR = os.path.join(DATA_DIR, 'library.sqlite3')
    ...
    def __init__(self) -> None:
        ...
        # DataBase
        self.connectionDB = sqlite3.connect(self.LIB_DIR)
    ...
    def updateDataBase(self):
        ...
        for record in take:
            ...
            if oper_id[4] == oper_id[6]:
                self.cur.execute(
                    f'UPDATE Journal SET Time_giveback=CURRENT_TIMESTAMP WHERE Amount_giveback={amount+int(record[2])} AND user_id={indexUser} AND book_id={int(record[0])} AND operation_id={oper_id[0]}')
...
""")
    ...
    def printout(self):
        ...
        doc = DocxTemplate(self.DOCX_DIR)
        context = {
